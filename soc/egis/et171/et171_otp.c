...
#define OTP_DATA_BAES                   0x1000
#define OTP_DATA_UID_0                  0x0000
#define OTP_DATA_UID_1                  0x0004
#define OTP_DATA_UID_2                  0x0008
#define OTP_DATA_ROOT_CLOCK             0x000C
#define OTP_DATA_ANALOG                 0x0018

#define READ_OTP_DATA(data_offset) \
	((uint32_t)sys_read32(OTP_REG_BASE + OTP_DATA_BAES + (data_offset)))


static otp_analog_option_t et171_otp_get_analog_config(void)
{
	otp_analog_option_t ret = { .__word = READ_OTP_DATA(OTP_DATA_ANALOG) };
	return ret;
}
...
{
	et171_otp_set_analog_config(et171_otp_get_analog_config());
}

uint32_t et171_otp_get_root_clock(void)
{
	return READ_OTP_DATA(OTP_DATA_ROOT_CLOCK);
}

void et171_otp_get_uid(uint8_t uid[16])
{
	((uint32_t *)uid)[0] = READ_OTP_DATA(OTP_DATA_UID_0);
	((uint32_t *)uid)[1] = READ_OTP_DATA(OTP_DATA_UID_1);
	((uint32_t *)uid)[2] = READ_OTP_DATA(OTP_DATA_UID_2);
	((uint32_t *)uid)[3] = 0;
}
